...
<<read, include=FALSE>>=
knitr::read_chunk("eduardo-quali.R")
@
<<setup, include=FALSE>>=
@
...
<<pmf-cmp, results="asis", fig.height=3, fig.width=7, fig.cap="Probabilidades pela distribuição COM-Poisson$_\\mu$ para diferentes parâmetros.">>=

include_graphics("images/pmf-cmp-1", auto_pdf = TRUE)
...
<<approx-plot, results="asis", out.width=".8\\textwidth", fig.height=3, fig.width=6, fig.cap="Erros quadráticos das aproximações para (a) média e (b) variância.">>=

include_graphics("images/approx-plot-1")
...
<<indexes-plot, results="asis", fig.width=9.5, fig.height=4, fig.cap="Índices para a distribuição COM-Poisson. (a) Relação média--variância, (b--d) Índices de dispersão, inflação de zeros e cauda pesada para diferentes parâmetros. As linhas tracejadas representam o caso especial Poisson.">>=

include_graphics("images/indexes-plot-1")
...
<<justpars, results="asis", fig.height=4, fig.width=9, fig.cap="Contagens médias (left) and índices de dispersão (right) para cada cenário considerado no estudo de simulação.">>=

include_graphics("images/justpars-1")
...
<<bias-plot, results="asis", fig.height=6.5, fig.width=8.5, fig.cap="Distribuições dos viéses padronizadas (box-plots em cinza) e média com intervalos de confiança (segmentos em preto) de cada diferente tamanho de amostra e nível de dispersão.">>=

include_graphics("images/bias-plot-1")
...
<<coverage-plot, results="asis", fig.height=4, fig.width=9, fig.cap="Taxas de cobertura dos intervalos de confiança obtidos por aproximação quadrática da verossimilhança para diferentes tamanhos de amostra e níveis de dispersão.">>=
include_graphics("images/coverage-plot-1")
...
<<ortho-plot, results="asis", fig.height=4.5, fig.width=9, fig.cap="Correlações empíricas entre os estimadores dos parâmetros de regressão e dispersão para cada tamanho de amostra e nível de dispersão.">>=

include_graphics("images/ortho-plot-1")

@

<<ortho-surf, results="asis", fig.height=5, fig.width=9, fig.cap="Gráficos de contorno das superfícies de deviance sob a parametrização original e proposta para quatro conjunto de dados simulados de tamanho 1000. As elipses representam as regiões de confiança de 90, 95 and 99\\%, as linhas tracejadas são as estimativas de máxima verossimilhança e os pontos são os valores dos parâmetros usados na simulação.">>=

include_graphics("images/ortho-surf-1")
