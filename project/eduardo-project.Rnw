...
aleatória). A \autoref{fig:processo-pontual}(a), representa a situação
de dados de contagem equidispersos. As ocorrências dos eventos se
dispõem aleatoriamente. Na \autoref{fig:processo-pontual}(b), o padrão
...
\autoref{fig:processo-pontual}(c), ilustra-se o caso de subdispersão, em
