...
O modelos lineares generalizados podem ser ajustados por um algoritmo
...
contagens, por exemplo, o modelo Poisson tem função de variância
$V(\mu_i) = \mu_i$, isto é, a esperança e a variância da distribuição
...
casos, os coeficientes ainda podem ser estimados consistentemente, porém
...
superdispersas leva à subestimação dos erros padrões das estimativas; e
para contagens subdispersas os erros padrões são superestimados.
...
podem levar à variabilidade extra Poisson. Dentre elas citam-se
...
aleatória). A \autoref{fig:processo-pontual}(a) representa a situação de
dados de contagem equidispersos. As ocorrências dos eventos se dispõem
aleatoriamente. Na \autoref{fig:processo-pontual}(b) o padrão já se
altera, tem-se a representação do caso de superdispersão. Formam-se
...
contagem de frutos apodrecidos). Na \autoref{fig:processo-pontual}(c)
ilustra-se o caso de subdispersão, em que as ocorrências se dispõem
uniformemente no espaço. Agora, as contagens de ocorrências nas parcelas
variam bem pouco. Ao contrário do caso superdisperso, uma causa
interpretável seria o oposto de contágio, a repulsa, ou seja, uma
ocorrência causa a repulsa de outras ocorrências em seu redor
(e.g. contagem de árvores) ou ainda competição (e.g. contagem de animais
territoriais ou que disputam por território).
...
xyplot(y ~ x | caso, data = da,
       ...
       scales = list(
           y = list(draw = FALSE),
           x = list(at = pretty(da$x),
                    labels = rep("", length(pretty(da$x))))
       ),
       xlab = "",
       ylab = "",
       strip = strip.custom(
       ...
       panel = function(x, y, ...) {
           ...
           panel.xyplot(x, y, ...)
           # grid.text(y = 0, sub[panel.number()])
       })
subs <- paste0("(", letters[1:3], ")")
poss <- c(0, 1, 2)/3 + 0.02
lapply(seq(poss), function(i) {
...
não equidipersos estão, geralmente, relacionadas às causas da não
...
Poisson-Gaussiano-inverso (PIG) e Neyman-Type A assumem que os efeitos
...
\citep{Consul1992, Zamani2012}, uma generalização bi-paramétrica do
modelo Poisson.
...
aleatórios em nível de grupos experimentais; tais modelos são denominados
...
literatura. \citet{Molenberghs2007} e \citet{Molenberghs2010} apresentam
uma nova família de modelos que consideram efeitos aleatórios normais,
para contemplar a estrutura hierárquica e conjugados, para contagens
condicionalmente superdispersas. No entanto, modelos hierárquicos mais
flexíveis, capazes de modelar diferentes níveis de dispersão das
contagens condicionais aos efeitos aleatórios, não são consolidadas na
literatura e são escassas suas aplicações.
...
utillizadas serão disponibilizadas, para que toda a pesquisa seja
...
\autoref{fig:desc-cotton}, são apresentados os números de capulhos
...
do modelo COM-Poisson e \citet{Bonat2017} apresentaram os resultados da
...
\autoref{fig:desc-soybean}, são apresentados os valores contabilizados de
...
<<desc-soybean, fig.width=9, fig.height=4, fig.cap="Dispersão do número de grão e de vagens viáveis observado para cada nível de umidade do solo e de adubação potássica. As linhas contínuas ligam os pontos médios de cada tratamento.">>=
...
\citet{RibeiroJr2018} mostram que há uma variabilidade extra
...
\autoref{fig:desc-epilepsy}, são apresentados os perfis do número de
...
\Sexpr{sum(epilepsy$nseizw == 0) / length(epilepsy$nseizw)*100}\% dos
dados observados.
...
registra-se $n_i$ observações ($n_i$, número de semanas em que o
...
Na \autoref{fig:desc-bromelia}(a), são apresentados os perfis
...
Sob essa motivação, \citet{Suekane2013} conduziu um experimento sob o
...
infestação. Na \autoref{fig:desc-whitefly}(b), apresentam-se as médias e
...
A \autoref{eqn:renewal} resulta uma distribuição de contagem a partir,
...
A distribuição \textit{Gamma-Count} GCT$(\alpha, \gamma)$ assume
distribuição gama, com de parâmetros $\alpha$ e $\gamma$, para
$\tau_k$. Consequentemente, a distribuição de $\vartheta_y$ também é
gama, porém com parâmetros $y\alpha$ e $\gamma$. Sua função massa de
...
superdispersão. \citet{Bonat2017} estendem essa distribuição para
...
da especicação das derivadas de $\ell(\bm{\theta})$, como BFGS
...
\citet{Bonat2017} propuseram o uso combinado das funções de estimação
...
acertivas nos experimentos conduzidos, principalmente, na área
    ...
    % \begin{SingleSpacing}
      ...
      \begin{tikzpicture}[thick,
        ...
        \begin{ganttchart}[
          ...
          progress=100]{Objetivo (i)}{1}{2} \\
          %-------------------------------------------
          % Segundo artigo
          \ganttgroup[group label node/.append style={align=right},
          progress=48]{Objetivo (ii)}{1}{10} \\
          ...
          % Terceiro artigo
          \ganttgroup[group label node/.append style={align=right},
          progress=15]{Objetivo (iii)}{10}{22} \\
          \ganttbar[progress=60]{
