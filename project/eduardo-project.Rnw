...
para Z$(\lambda, \nu)$, \citeonline{Shmueli2005} e
\citeonline{Sellers2010} mostram que a esperança e variância de uma
...
não são obtidos em forma fechada. \citeonline{Winkelmann1995} mostra que
...
negativa $(p=2)$ e Poisson inversa gaussiana $(p = 3)$ \cite{Bonat2017}.
...
especificação de momentos \cite{Wedderburn1974} baseada nas expressões
