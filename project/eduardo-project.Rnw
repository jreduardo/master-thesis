...
contagens e de tempo até o evento \cite{Molenberghs2010}. Para dados de
...
de dependência de duração \cite{Winkelmann1995}. Outra possível causa da
...
cultura do algodão \textit{Gossypium hirsutum}, \citeonline{Silva2012}
...
dados. \citeonline{Zeviani2014} utilizou o modelo Gamma-Count,
\citeonline{Huang2017} e \citeonline{RibeiroJr2018} utilizaram
reparametrizações do modelo COM-Poisson e \citeonline{Bonat2017}
...
um delineamento de blocos casualizados, \citeonline{Serafim2012} avaliou
...
\citeonline{RibeiroJr2018} analisou o número de grãos por parcela com
...
\citeonline{RibeiroJr2018} mostra que há uma variabilidade extra
...
medicina. \citeonline{Faught1996} apresenta um estudo aleatorizado,
...
médio e mediano. Como destacam \citeonline{Molenberghs2007}, é
...
indivíduo. \citeonline{Molenberghs2005} utiliza esse conjunto de dados
...
estimação generalizadas). \citeonline{Molenberghs2007} e
\citeonline{Molenberghs2010} também utilizam esses dados como exemplo de
...
econômica dessa praga em agronômia.
...
Sob essa motivação, \citeonline{Suekane2013} conduziu um experimento sob
...
\cite{Jorgensen1997}. A distribuição de Poisson Po$(\mu)$, com média
...
probabilidades consecutivas $\Pr(Y=y-1)/\Pr(Y=y-1)$ e a relação da
distribuição Poisson com a distribuição exponencial são características
que motivam classes de generalizações do modelo Poisson e serão
discutidas nas próximas seções.

Para especificar um modelo de rergessão baseado na distribuição Poisson,
considere $y_i$ observações de uma variável $Y_i$ Poisson distribuída
e $\bm{x}_i^\top = (x_{i1}, x_{i2}, \ldots, x_{ip})$ um vetor de
covariáveis conhecidas, $i=1,2,\ldots,n$.
O modelo de regressão Poisson é especificado como
$$
...
distribuições Poisson ponderadas (WPD) \cite{DelCastillo1998}. Uma
variável aleatória $Y$ pertence à familía WPD se sua função massa de
...
$\nu \geq 0$ \cite{Sellers2012}. Sua função de probabilidade assume a
forma
\begin{equation}
...
limite a distribuição Bernoulli ($nu\to \infty$ com probabilidade de
sucesso $\lambda/(\lambda+1)$).

Um inconveniente desse modelo é que os momentos esperança e variância,
em geral, não são obtidos em forma fechada. \citeonline{Shmueli2005} e
\citeonline{Sellers2010} a partir de uma aproximação para
Z$(\lambda, \nu)$, mostram que a esperança e variância de uma variável
...
\cite{Shmueli2005}. \citeonline{RibeiroJr2018} mostrou que a aproximação
para a esperança é acurada, diferentemente da aproximação para
variância, que perde acurácia quando $\nu<1$. Além disso, o autor indica
que a acurácia das aproximaççoes parecem não ter relação com as regiões
$\nu \leq 1$ ou $\lambda > 10^\nu$.

Modelos de regressão baseados na distribuição COM-Poisson foram
propostos por \citeonline{Sellers2010}. Para $y_i$ observações
...
\cite{RibeiroJr2018}. Sob essas motivações \citeonline{Huang2017} e
\citeonline{RibeiroJr2018} propuseram reparametrizações para a média no
modelo COM-Poisson. Nesses caso, o modelo de regressão é especificado
como
$$
...
\citeauthor{Huang2017}, o parâmetro $\mu$ da distribuição
$\text{CMP}_\mu(\mu_i, \nu)$ é obtido como raíz da equação
$$ \sum_{j=0}^{\infty} (j-\mu_i) \frac{\lambda^j}{(j!)^\nu} = 0. $$
Já na proposta de \citeauthor{RibeiroJr2018}, $\mu$ é obtido a partir da
aproximação para a esperança,
$$ \mu_i = h(\lambda, \nu) = \lambda^{1/\nu} - \frac{\nu - 1}{2\nu}. $$
...
derivada da relação da Poisson com a distribuição do tempo entre
...
$n=1,2,\ldots$. Seguindo \citeonline{Winkelmann1995}, seja $Y_T$ o
...
$y$ e $\lambda$, e a função massa de probabilidade é obtida como
$$
...
gamma distribuídos, com de parâmetros $\alpha$ e $\gamma$. Assim
$\vartheta_y$ também é gamma distribuído, porém com parâmetros $y\alpha$
e $\gamma$. Sua função massa de probabilidade é obtida por
\begin{equation}
...
a distribuição se reduz a Poisson. Os momentos da distribuição também
não são obtidos de forma fechada. \citeonline{Winkelmann1995} mostra que
...
modelar superdispersão para $0 < \alpha < 1$ e subdispersão para
$\alpha > 1$.

Modelos de regressão Gamma-Count foram propostos por
\citeonline{Winkelmann1995} e \citeonline{Zeviani2014}.  Para $y_i$
...
A desvantagem desse modelo é que, embora assintaticamente, para grandes
valores de $T$, $\gamma_i / \alpha$ represente a média das contagens, em
geral modela-se na escala do tempo entre eventos, perdendo a
interpretação dos parâmetros de regressão $\beta$.
...
superdispersão \cite{Zamani2012}. Seja $Y$ variável aleatória que segue
...
negativo \cite{Consul1992}. A média e variância são
...
\begin{equation}
  ...
  \Pr(Y=y) = \left ( \frac{\mu}{1+\alpha \mu} \right )^y
    ...
    \right ],
\end{equation}
denotada por $\text{GPo}_\mu(\mu, \alpha)$. Para $y_i$ observações da
distribuição Poisson generalizada e $\bm{x}_i$ um vetor de covariáveis
conhecidas, o modelo de regressão baseado na distribuição Poisson
generalizada é definido por
$$
...
reportadas na literatura. A grande dificuldade deste modelo é a
complicada restrição no espaço paramétrico, que é difícil de se
incorporar, de forma eficiente, no processo de estimação.

\section{Distribuição Poisson-Tweedie}
\label{sec:poisson-tweedie}

Distribuições de contagem baseadas em misturas da distribuição são
comuns para análise de dados superdispersos. A distribuição
Poisson-Tweedie representa um caso geral de misturas Poisson
\cite{Jorgensen1997}. Ela consiste da inclusão de aleatórios aleatórios
Tweedie distribuídos em nível de observação.

A distribuição Tweedie pertence a classe dos modelos exponenciais de
dispersão \cite{Jorgensen1997}. Seja E$(Y = \mu)$ e
...
distribuições uma vez que as distribuições Normal $(p = 0)$, Poisson
$(p = 1)$, non-central gamma $(p = 3/2)$, gamma $(p = 2)$ and inverse
Gaussian $(p = 3)$ são casos particulares. A função densidade de
...
$(p = 0)$, Poisson compound Poisson $(1 < p < 2)$, binomial negativa
$(p=2)$ e Poisson inversa gaussiana $(p = 3)$ \cite{Bonat2017}.
...
Y_i \sim \text{PTw}_p(\mu_i, \phi), \quad \text{em que} \quad
  \mu_i = g^{-1}(\bm{x}_i^\top\bm{\beta}),
$$
e em sua versão estendida, a especificação fica
$$
...
dados de contagem superdispersos. \citeonline{Bonat2017} estendem essa
distribuição para lidar com subdisprsão, adotando apenas a especificação
dos momentos (\ref{eqn:moments-ptw}) e permitindo a estimação de
$\phi<0$. Sob essa abordagem, não se conhece a distribuição completa de
$Y$, impossibilitando o cálculo de probabilidades, por exemplo.

A grande motivação da utilização dessa distribuição é sua função de
variância, bastante flexível, onde a estimação de $p$ funciona como um
selecionador automático de distribuição.

\section{Estimação e inferência em modelos de regressão}
\label{sec:estimacao-e-inferencia}

Para as versões parâmetricas dos modelos apresentados anteriormente, o
processo de estimação pose ser feito sob o paradigma de verossimilhança,
que é bastante intuitivo e fornece estimadores consistentes e
assintoticamente não viesados \cite{Pawitan2001}. Para $\bm{y}$ um
...
\cite{Bonat2017b}. Entretanto, é usual assumir as estimativas de
...
derivatives-free, como BFGS \cite{Nocedal1995}, para quando
...
/ \partial \beta_p)^\top$ o vetor de derivadas primeiras.
...
\citeonline{Bonat2017} propôs o uso combinado das funções de estimação
