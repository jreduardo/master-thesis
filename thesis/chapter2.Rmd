...
and $\lambda<1$ ($\Pr(Y=y) = \lambda^y(1-\lambda)$). As a limiting case,
...
and three special cases of three-parameter Poisson-Tweedie distribution:
Poisson compound Poisson ($p=1.1$), negative binomial ($p=2$) and
Poisson inverse-Gaussian ($p=3$). The dispersion parameters of the count
...
present only the other distributions. For the CMP distribution, we could
not find the parameter values to have dispersion index equal to 10 when
...
we can model only overdispersed. Therefore, we use only the second-order
...
generating probability distributions and the main properties is
...
dependence, the predicted values for the random effects were very close
to zero.
