...
is illustrated with the analysis of two experimental data sets. The
...
and $\lambda<1$ ($P(Y=y) = \lambda^y(1-\lambda)$). As a limiting case,
the Bernoulli distribution arises when $\nu\to \infty$, with probability
of success equal to $\lambda/(\lambda+1)$.
...
when $\rho=1$ (the popular geometric distribution) and discrete Rayleigh
($\rho=2$).
...
negative binomial distribution [@Zamani2012]. Let $Y$ be a random
...
largest integer value for which $\lambda + m\gamma >0$ when $\gamma$
...
(Equation \ref{eqn:chap2-espec-ptw}), the mean and variance can
easily be obtained,
\begin{equation}
...
Poisson (DPo) distributions, the expected values and the variances are
...
following the codes provide by @Bonat2018. For the CMP distribution, we
could not find the parameter values to have dispersion index equal to 10
...
suitable to handle with very strong overdispersion. For the DWe
distribution, the $x$-axis is the $q$ parameter, so the interpretation
...
tends to 0 as the mean increases, hence the $\text{ZI}\to -Inf$. That
...
Note that this specification allows us to handle under- and
...
Note that for the PTw model, we could calculate the log-likelihood
function since the estimated parameters ($\hat{p}=1.4031$ and
$\hat{\omega}=0.3476$) correspond to an existing Poisson-Tweedie
distribution. Furthermore, except for DWe model, all models have large
...
parameter) for the models nesting the Poisson model (i.e. CMP, GCT, GPo,
DPo and PTw) indicates strong evidence to reject the null hypothesis.
...
not suitable to fit to this dataset.
...
time when it is suitable for commercialization. We assumed $Y_{ij}$
...
Poisson-Tweedie is based only on the first and second moments. The
estimation method implemented in `mcglm` package crashes to fit to this
dataset when considering free power parameter. However, to fixed power
parameter between $0.93$ and $1.1$ we are able to estimate $\omega$ and
$\bm{\beta}$. In Figure \@ref(fig:chap2-plot-ptwprofile-bromelia), we
show the profile pseudo-log-likelihood for the power parameter $p$,
considering the three linear predictors. The results suggest that a
special case Neyman Type A (NTA) ($p=1$) could be a good choice to fit
to this dataset.
...
dispersed count data and compare them using characteristic indexes
