...
model. Double Poisson distribution proposed by @Efron1986 is used as the
random component in general purposed class of models GAMLSS (generalized
additive models for location, scale, and shape) [@Rigby2005]. @Bonat2018
proposed the extended Poisson-Tweedie for handling under- and
overdispersion count data based on moments of the Poisson-Tweedie
family.

The main objective of this chapter is compare several model strategies
for analysis of dispersed count data, namely COM-Poisson, Gamma-Count,
...
motivation we showed in Chapter \@ref(chap1-reparcmp) the @Huang2017
and @RibeiroJr2018 proposals of new parametrizations of COM-Poisson
distribution related to the expectation. The distribution is now indexed
by parameter $\mu$, where $\mu$ is obtained by the solution for
$\sum_{j=0}^{\infty} (j-\mu)\lambda^j/(j!)^\nu = 0$ in the Huang's
...
no genuine dispersion and location parameter. The dispersion is
controlled by both $q$ and $\rho$.
...
\text{E}(Y)=\sum_{y=0}^{500} \{[y-\text{E}(Y)]^2\}\Pr(Y=y)
$$
