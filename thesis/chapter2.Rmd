...
count distributions that can handle both under- and overdispersion has
...
distributions for practical data analysis. The main properties of the
distributions are highlighted and compared considering the dispersion,
zero-inflation, and heavy tail indexes. The application of these models
is illustrated with the analysis of two experimental data. The
computational routines for fitting the models and the data sets are
...
levels of dispersion, since the standard Poisson generalized linear
model supposes that the variance equals the mean, referred to as
equidispersion, which rarely occurs in real data. A comprehensive review
of new methods for modeling count data can be found in the recent
monographs by  @Winkelmann2008, @Cameron2013, and @Hilbe2014.

The most common failure of the equidispersion assumption is the
overdispersion (or extra-Poisson dispersion) when the variance exceeds
...
when the variance is less than the mean, is less often reported in the
literature. However, in the last decade, it has been of increasing
interest.
...
@Luyts2018 extend it to model longitudinal/clustered data. @Zamani2012
presented an extension for the generalized Poisson regression
model. Double Poisson distribution proposed by @Efron1986, is used as
the random component in general purposed class of models GAMLSS
(generalized additive models for location, scale, and shape)
[@Rigby2005]. @Bonat2018 proposed the extended Poisson-Tweedie for
handling under- and overdispersion count data based on moments of the
Poisson-Tweedie family.
...
The considered model strategies and their main properties are presented
in Section \@ref(chap2-background). In Section
\@ref(chap2-comparing-distributions), we presented the characteristic
index and use them to compare the models. The regression models and
estimation are presented in Section
\@ref(chap2-models-and-estimation). Section \@ref(chap2-data-analyses)
is devoted to illustrating the application of the models to two real
experimental datasets and compare the results. Finally, discussion and
direction for future work are given in Section
\@ref(chap2-discussion). The computational routines used in this chapter
are available in the supplementary material.

## Background ## {#chap2-background}

In this section, we shall introduced some count distributions take into
...
Poisson-Tweedie distributions. We focused on the genesis of generating
...
allowing for a non-linear decrease in ratios of successive
...
parameters $\lambda$ and $\nu$ are interpreted through ratios of
successive probabilities which is given, for the WPD family and
particularly for the COM-Poisson distribution, by
$$
...
1$ and $\nu > 1$ we have the over- and undersipersion,
respectively. When $\nu=1$ the Poisson distribution results as a special
case. Another special case is geometric distribution, when $\nu=0$ and
$\lambda<1$ ($P(Y=y) = \lambda^y(1-\lambda)$). As a limiting case, the
Bernoulli distribution arises when $\nu\to \infty$, with probability of
success equal to $\lambda/(\lambda+1)$.
...
motivation, @Huang2017 and @RibeiroJr2018 proposed new
parametrizations of COM-Poisson distribution related to the
expectation. The distribution is now indexed by parameter $\mu$, where
$\mu$ is obtained by the solution for $\sum_{j=0}^{\infty}
(j-\mu)\lambda^j/(j!)^\nu = 0$ in the Huang's proposal and $\mu =
\lambda^{1/\nu} -(\nu-1)/2\nu$ in the Ribeiro Jr's proposal.
...
\overset{\textit{\tiny iid}}{\sim} \text{Gamma}(\alpha, \beta)$. Due
to reproductive property of gamma random variables, $\vartheta_n \sim
\text{Gamma}(y\alpha, \beta)$. Thereby, the Gamma-Count probability
...
\end{equation}
a difference bettwen two Gamma cumulative density functions, $G(y\alpha,
\beta) - G((y+1)\alpha, \beta)$, where $G(a, b)$ is the cumulative
function $\text{F}_y(T)$ for the Gamma variable with parameters $a$ and
$b$.
...
\@ref(eqn:chap2-pmf-gct0) in terms of $\kappa = \beta/\alpha$, thus the
...
holds that the expected value $\text{E(Y)} = \kappa$.
...
the continuous Weibull distribution [@Nakagawa1975]. Let $T$ a
...
$$
The discrete Weibull distribution (type 1) by [@Nakagawa1975] is
obtained by the probability between two integer values on the suporte of
continuous Weibull distribution. Let $Y \in \mathbb{N}$ follows a
...
This discretization method can be used to discrete any continuous
distribution. From the special cases of Weibull, we have correspondents
specials cases discrete exponential when $\rho=1$ (the popular geometric
distribution) and discrete Rayleigh ($\rho=2$).
...
\begin{equation}
  ...
  \Pr(y) = \left ( \frac{\mu}{1+\sigma \mu} \right )^y
    ...
    \right ],
\end{equation}
\noindent where $\sigma > \text{min}(-1/y, -1/\mu)$, when
$\sigma<0$. The moments under this parametrization are given by
\begin{equation}
...
challenged by the presence of unusual parameter space for
underdispersion.
...
distributed has the exact probability mass function given by
\begin{equation}
...
The closed form approximation to the $K(\mu, \varphi )$ (Equation
...
[@Zou2013]. In this work, the normalizing constant is calculated by
truncating the series (i.e., the sum of the first $k$ terms).

The expected value and the variance referring to the double Poisson
distribution has no closed form, but @Efron1986 shows that it can be
approximated as
$$
\text{E}(Y) = \mu\quad \text{and}\quad
\text{Var}(Y) = \varphi \mu.
...
disadvantages of applying double Poisson to count data analysis are
...
\end{equation*}
where $\mu = k^{\prime}_p(\psi)$ is the expectation, $\phi > 0$ is the
dispersion parameter, $\psi$ is the canonical parameter and $k_p(\psi)$
is the cumulant function.  Furthermore, $\mathrm{var}(Z) = \phi V(\mu)$
...
by integrating out $z$,
\begin{equation}
  \label{eqn:chap2-pmf-ptw}
  \Pr(Y = y) = \int_0^\infty \frac{z^y \exp(-z)}{y!}
     a(z,\phi,p) \exp\{(z\psi - k_p(\psi))/\phi\} dz.
\end{equation}

The probability distribution \@ref(eqn:chap2-pmf-ptw) cannot be written
in a closed form, apart from the special case negative binomial
distribution ($p=2$). However, due to hierarchical specification
(Equation \ref{eqn:chap2-espec-ptw}), the moments mean and variance
...
The Poisson-Tweedie model can deal with only overdispersion
($\omega>0$). However, @Bonat2018 noted that variance can be less than
the expectation allowing $\omega<0$. So, they proposed the extended
...
($\text{HI}$), which are respectively given by
\begin{equation*}
...
Figures \@ref(fig:chap2-plot-dispersion-index),
\@ref(fig:chap2-plot-zeroinflation-index) and
\@ref(fig:chap2-plot-heavytail-index), display the indexes by different
scenarios of dispersion and count distributions. We considered all
two-parameter distributions described in Section \@ref(chap2-background)
and three special cases of Poisson-Tweedie distribution: Poisson
compound Poisson ($p=1.1$), negative binomial ($p=2$) and Poisson
inverse-Gaussian ($p=3$). The dispersion parameters of the count
distributions were set to have $\text{DI}=0.2, 0.5, 1,2, 5$ and $10$
when the means equals 10 and the location parameters were considered as
a sequence to have the expected values between 0 and 50. For the
COM-Poisson (CMP), Gamma-Count (GCT), discrete-Weibull (DWe) and double
Poisson (DPo) distribution, the expected values and the variances are
...
distributions, the expected values and the variances have a closed form
(Equation \@ref(eqn:chap2-moments-gpo) and Equation
\@ref(eqn:chap2-moments-gpo)). To obtain the HT and ZI for the
Poisson-Tweedie distributions, we approximate the value of integral
\@ref(eqn:chap2-pmf-ptw) using the Gauss-Laguerre method with 100 nodes,
following the codes provide to @Bonat2018. For the CMP distribution, we
could not find the parameter values to have dispersion index equals 10
when the expected value is 10. Since the Poisson-Tweedie cannot deal
with underdispersion, in this scenario we present only the other
distributions.
...
similar. In general, for these distributions, the indexes slightly
depend on the expected values and tend to stabilize for large values
of. Consequently, the mean-variance relationship is proportional to the
dispersion parameter value. In fact, if we look at the (approximate)
variances for these distributions, they follow the same structure. The
DIs for PTw$_3$ and GPo distributions shows that distributions are suitable
to handle with very strong overdispersion. For the DWe distribution, the
$x$-axis is the $q$ parameter, so the interpretations are not
straightforward.

The ZIs in Figure \@ref(fig:chap2-plot-zeroinflation-index) show that
CMP, GCT, DWe, GPo, and DPo can handle a limited amount of
zero-inflation, in cases of overdispersion ($\text{DI}<1$) and is
suitable to zero-deflation, in cases of underdispersion
($\text{DI}>1$). For the GCT distribution on the underdispersion
scenarios ($\text{DI}=0.5$ and $\text{DI}=0.5$), the probability at zero
tends to 0 with increasing the mean, hence the $\text{ZI}\to -Inf$. That
...
no distribution when $\mu > -1/\sigma$ (Equation
\@ref(eqn:chap2-pmf-gpo)). The ZI's for PTw distributions increase
quickly as the mean increases, indicating that distributions are is
...
\infty$. Actually, the HI for the DWe distributions tends to 1 for small
...
Suppose $y_i$, $i=1,\ldots,n$ a sample of independent counts according
to a probabilistic distribution $\mathcal{D}(. ,.)$ and $\bm{x}_i$ a
...
limiting) can be directly linked to the results presents in Figure
...
be obtained analytically for any of this models, we compute them by
central finite differences using the Richardson method as implemented in
package `numDeriv` [@numDeriv-pkg] in `R` [@Rcore2018]. The
computational routines for fitting these models are implemented in `R`,
using `C++` to compute the normalizing constant for CMP and DPo, and
organized in a `R` package
...
parameter space makes the estimation by maximum likelihood method hard
to use. Furthermore, by using the maximum likelihood method we can model
only overdispersed. Therefore, we use only the second-order moment
assumptions for this class, i.e.
$\text{E}(Y_i) = g^{-1}(\bm{x}_i^\top \bm{\beta}) = \mu_i$ and
$\text{Var}(Y_i) = \mu_i + \omega\mu_i^p$.
Note that this specification allows us to handle with under- and
overdispersion and eliminate the non-trivial constraint in the power
...
code for their analysis are available in the supplementary material.
...
linear predictor $\eta_{ij} = \beta_0 + \tau_i$, where $i$ indicates the
repetition and $j$ the treatment.

The parameter estimates, associated standard errors and goodness-of-fit
measures (minus twice maximized log-likelihood and Akaike information
criterion) are given in Table \@ref(tab:chap2-coeftable-sitophilus).
Although the PTw model has being fitted using only second-order moments
assumptions, the model parameters are estimated where the corresponding
Tweedie distribution exist ($\hat{p}=1.4031$ and $\hat{\omega}=0.3476$)
it being possible to calculate the log-likelihood and the AIC as
shown. Furthermore, except for DWe model, all models have large
improvements related to the classical Poisson model, whose maximum
log-likelihood is equal to $-130.5828$. The likelihood ratio tests for
H$_0: \log(\theta)=0$ (where $\theta$ is the associated dispersion
parameter) for the models nested to the Poisson model (i.e. CMP, GCT,
GPo, DPo and PTw) indicates strong evidence to reject the null
hypothesis.
...
(control, with leaves and with branch) had the same effect (see
regression coefficients in Table \@ref(tab:chap2-coeftable-sitophilus).
The same conclusion is reached by @Demetrio2014 using a quasi-Poisson
...
asymptotically, the DWe model parameters are on a not easily interpreted
scale.
...
$g^{-1}(\eta_{\text{upr}})$ (lower and upper of the confidence interval),
and dispersion parameter fixed at the maximum likelihood estimate. The
results show that the variance increases linearly for CMP, GCT and DPo
and polynomially for DWe, GPo and PTw. In fact, the polynomials for GPo
and PTw are $f(x)=x(1+x/50)^2$ and $f(x)=x+7x^{1.03}/20$, respectively.
...
avoid the non-linearity (sigmoidal) relationship between response and
...
time when it is suitable for commercialization. We assumed $Y_ij$
distributed according to the six flexible distributions and consider the
...
\end{tabular}
\noindent
where $i$, $j$, and $k$ refers to the block, treatment and time,
...
in Section \@ref(chap0-bromelia) suggesting strong
underdispersion. Thus, Poisson-Tweedie is based only on the first and
second moments. The estimation method implemented in `mcglm` package
crashes to fit this data when considering free power parameter. However,
to fixed power parameter between $0.93$ and $1.1$ we are able to
estimate $\omega$ and $\bm{\beta}$. In Figure
...
(NTA) ($p=1$) can be a good choice to analyze this data.
...
Table in terms of global deviance (minus twice log-likelihood). The
global deviance obtained from the Poisson model with the linear
predictor varying the quadratic term is equal to $439.4984$ with 82
degrees of fredoom and the usual deviance is equal to $1.82$. Here there
is clearly overwhelming evidence against the Poisson assumption. The
CMP, GCT and DPo models fit the data better than the DWe and GPo models
in terms of likelihood. The latter model gives a poor fit for the
underdispersion. In practical terms, this leads to different conclusions
about the hypothesis H$_0$: $\beta_{1j}=0$. For CMP, GCT, DWe and DPo
models there is evidence to reject this hypothesis whereas for GPo there
is no evidence to reject H$_0$.
...
parametric models and for the NTA models (Poisson-Tweedie with fixed
...
to note that $\hat{\nu} \approx 1/\hat{\phi} \approx \hat{\alpha}$.
...
underdispersion of this data. Therefore, except for the GPo models, the
individual Wald tests indicate that intercept, linear and quadratic
terms are different for each substrate.

```{r chap2-fitted-plot-bromelia, results="asis", fig.width=9, fig.height=4.5, fig.cap="(a) Scatterplots of the observed data and fitted values with 95\\% confidence bands and (b) Fitted curves for each treatment.."}
...
each alternative substrate. The results support that the growth rate of
the expected number of leaves is higher for the bromeliads grown with
the substrate composite with \textit{Pinus} bark, being this the
preferred substrate to replace the composite with Xaxim.
...
DWe. The NTA gives slightly larger intervals than others, this is
...
dispersed count data and compare them throughout characteristic indexes
and applications to the analysis of two experimental data. The genesis
of generating probability distributions and the main properties is
...
double Poisson models are indistinguishable in terms of mean-variance
relationship. Moreover, the study of zero-inflation and heavy-tail
indexes confirm the similarity. The COM-Poisson and double Poisson
models are weighted-type distributions, so the computation of
...
of your flexibility; it seems that is a distribution useful for small
...
although it deals with underdispersion, its unusual parametric space
...
distribution for underdispersion makes impossible to compute
...
estimation difficult. In such cases, the choice of power parameter can
be made based on the profile (or pseudo-profile) log-likelihood.

Regarding the analysis of experimental data, for the first experiment
does, it is clear that the extract prepared with seeds is different from
the others and the other solution has no difference even without fit any
...
dependence, the values predicted for the random effects were very close
to zero.
...
there is a biological process leading to non-linear models, and the
...
Weibull not seems good to modeling, thus investigating new
parametrizations can be useful. Finally, allow the dispersion parameter
to be modeled to depend on covariates is a natural extension.
