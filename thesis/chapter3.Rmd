...
dispersion can produce a loss of efficiency. In this chapter, we
proposed an extension of the COM-Poisson model to jointly model the mean
and the dispersion. This approach allows the data exhibit under- and
overdispersion depending on covariates. The estimation and inference are
based on likelihood method. The application of the COM-Poisson models
with varying dispersion is illustrated with the analysis of two
experimental data. The computational routines for fitting the models and
the datasets are available in the upplementary material.
 \vspace*{0.3cm}
...
The classical linear (Gaussian) models are based on the assumption of
variance homogeneity [@Aitkin1987]. Generalized linear models relax this
assumption by assuming the observations distributed according to some
distribution in the exponential family [@Nelder1972]. A key feature of
exponential family distribution is the so-called mean-variance
relationship, i.e. the variance is a deterministic function of the mean
$V(Y) = \phi V(\mu)$. The main examples are $V(\mu) = \mu(1-\mu)$ for
the binomial distribution, $V(\mu) = \mu$ for the Poisson distribution,
...
specific heterogeneity by assumes an appropriate distribution. However,
once the mean-variance relationship is specified, the variance is
assumed known up to a constant of proportionality, the dispersion
parameter $\phi$. This allows modeling specific heterogeneity by assumes
an appropriate distribution. However, once the mean-variance
relationship is specified, the variance is assumed known up to a
constant of proportionality, the dispersion parameter $\phi$. To ensure
more flexibility in the analysis of non-Gaussian heterogeneous data, we
explore methods to model dispersion depending on covariates. We focus on
count data only.

Modeling dispersion depending on covariates in the analysis of count
data is not as explored in the literature. The class of double
generalized linear models [@Smyth1988; @McCullagh1989; Smyth1999] can be
used to do it. This class was widely explored for continuous
data. @Smyth1988 discussed this class using a double generalized linear
model based on the gamma distribution. The adjusted method for
estimation and inference are presented by @Smyth1999, also with
application to the analysis of continuous data. @Paula2013 discussed
diagnostics for this class and used a gamma distributed data as an
application. @Andersen2017 extended the double generalized linear model
in considering compound Poisson distributions for continuous
data. Related to discrete data, @Vieira2011 proposed a Bayesian analysis
of the double generalized linear models for binomial data.
...
In this chapter, we propose to jointly model the mean and the dispersion
based on the COM-Poisson distribution. This approach is very similar to
the GAMLSS, however, we developed and explored our own estimation
methods. This approach allows the data exhibit under- and overdispersion
depending on experimental conditions.

This chapter is organized as follows. In Section \@ref(chap3-dglm) we
briefly described and commented the double generalized linear
models. The newly proposed COM-Poisson models is considered in Section
\@ref(chap3-cmpreg). In Section \@ref(chap3-estimation-and-inference) we
present estimation and inference forthe COM-Poisson regression model in
a likelihood framework. Section \@ref(chap3-applications-and-discussion)
is devoted to illustrate the application of the COM-Poisson model with
varying dispersion with the analysis of two data sets. We compared the
results of COM-Poisson model with double generalized linear models and
GAMLSS approach. Finally, concluding remarks close the chapter in
Section \@ref(chap3-final-remarks). We provide an `R` implementation for
fitting the COM-Poisson and reparameterized COM-Poisson models with
varying dispersion, together with the analyzed data sets, in the
...
models (DGLM) model mean and dispersion jointly by considering two
linked generalized linear models (GLM). Let $y_i$, $i=1,2,\ldots,n$ be a
set of independent observations and $\bm{x}_i = (x_{i1}, x_{i2}, \ldots,
x_{np})^\top$ a vector of known covariates, the standard GLMs assume
that
$$
\text{E}(Y_i) = \mu_i, \quad
...
function. The DGLMs generalize the GLM to allow the dispersion parameter
varying with the observations, $\text{Var}(Y_i)=\phi_i V(\mu_i)$. The
submodel for dispersion is specified by
$$
...
link function. The commom choice for $d_i$ is the deviance components of
the mean submodel. And, based on asymptotic distribution of $d_i$, a
gamma model with $V_d(\phi_i) = 2\phi_i^2$ is a suitable natural choice
for the dispersion submodel.

Parameter estimation proposed is based on a two-step
...
different names. @Smyth1988 called it by generalized linear models with
varying dispersion. While @McCullagh1989, at the same time, named this
approach by joint modelling of mean and dispersion. Nowadays, most used
name to refer to this approach is double generalized linear models,
...
require a two-parameter distribution to the response. In this chapter,
we compare the proposed COM-Poisson model with varying dispersion with a
GAMLSS based on the double Poisson distribution [@Efron1986; @Zou2013]
and a DGLM Poisson.

## Modeling of mean and dispersion COM-Poisson ## {#chap3-cmpreg}

The COM-Poisson distributions is a two-parameter generalization of the
...
mean. In this paper, we propose to allow both $\mu$ and $\phi$
parameters to depend on covariates.

Let $y_i$, $i=1,2,\ldots,n$ be a set of independents realizations of
$Y_i$ according to COM-Poisson distribution with parameters $\mu_i$ and
...
functions. We use the logarithm link function for both mean and
dispersion.
...
advantages of allowing inference, obtaining predictions for
probabilities and facilitating generalizations such as random effects or
modeling of censored data.
...
$\bm{\theta}^\top = (\bm{\beta}^\top, \bm{\gamma}^\top)^\top$ parameters
are given by
\begin{equation}
...
\exp(\bm{x}_i^\top \bm{\gamma})$, and $Z(\mu_i, \nu_i)$ is a normalizing
constant computed for the parameters $\mu_i$ and $\nu_i$.

Parameter estimation requires the numerical maximization of Equation
\@ref(eqn:ll-cmpvarying). Since the derivatives of cannot be obtained in
closed forms, we compute them by central finite differences using the
Richardson method as implemented in package `numDeriv` [@numDeriv-pkg]
for the statistical software `R` [@Rcore2018]. Based on the
orthogonality property between $\bm{\mu}_i$ and $\bm{\nu}_i$ [see
@RibeiroJr2018], we set two strategies to obtain the maximum likelihood
estimates:
\vspace{-0.2cm}
\begin{itemize}
  ...
  \item[(b)] the \textit{conditional strategy}, where we fixed
...
Although we have an additional step of estimating the Hessian matrix,
the strategy fixed tends to be faster than joint, because of the
maximization is performed in $\mathbb{R}^q$ whereas, for strategy joint,
...
estimators, we have
$$
\text{Var}(\hat{\bm{\beta}}) = \bm{V}_{\beta} -
  (\bm{V}_{\beta\gamma} \bm{V}_{\gamma}^{-1})^\top
   \bm{V}_{\beta\gamma}\quad\text{and}\quad
\text{Var}(\hat{\bm{\gamma}}) = \bm{V}_{\gamma} -
...
in this case, $\text{Var}(\hat{\bm{\beta}}) = \bm{V}_{\beta}$ and
$\text{Var}(\hat{\bm{\gamma}}) = \bm{V}_{\gamma}$.
...
DPO)`. The data sets and `R` codes used in this section are available as
supplementary material.
...
$\bm{\texttt{x}}_1$ and $\bm{\texttt{x}}_3$ are the orthogonal linear,
...
\begin{table}[ht]
  \centering
  \caption{Nitrofen data: Bromelia data: goodness-of-fit measures (deviance and AIC) and model comparisons.}
...
the models using the joint strategy, for that reason the parameter
estimates for mean structure are slightly different for the considered
...
adopted for the mean are not the saturated one, the fitted curve almost
...
mu_i and nu_i, so a linear predictor of $\nu_i$ does not imply a linear
curve here. The model with a linear predictor for the dispersion does
not fit the sample variances for the doses 1.6 and
2.35$\mu$g$/10^2$litre well. The model with a quadratic predictor fit
these variances better, however, produces uncommon behavior after
2.35$\mu$g$/10^2$litre.
...
we focus on analyzing the number of pods. Here we focus on analyzing the
number of pods and consider the following predictors for mean and
dispersion
$$
\begin{aligned}
...
measures (deviance, minus two times the maximized log-likelihood and
Akaike information criterion) together with the corresponding likelihood
ratio tests between the current model (moisture + K) and the reduced
models for dispersion where $\gamma_1=0$ (moisture effect) and where
$\alpha_j = \alpha$ (constant). The log-likelihood achieved for the
COM-Poisson, double Poisson and double generalized linear Poisson models
are practically the same for all three predictors for dispersion. The
results show a significant improvement in varying the dispersion for
each moisture level and do not present a clear relation between the
dispersion and the potassium doses when fitted to the moisture levels.
...
lead to the same conclusions for the different dispersion structures. As
well as the likelihood ratio tests, the results indicate that we have
different dispersion levels for each moisture level. In particular,
considering the reduced model $\gamma_1=0$, the fitted dispersion for
the moisture level $37.5$\%, $50.0$\%, and $63.5$\% is $\nu=0.701$,
$0.742$, and $1.99$, respectively. The point estimates indicate
overdispersion for the first two and underdispersion for the latter.
...
component. The results do indicate neither under- nor overdispersion for
...
fertilization with potassium can prevent water deficit in soybean
culture.
...
asymptote can be very useful with direct interpretations.

## Final remarks ## {#chap3-final-remarks}

In this paper, COM-Poisson models with varying dispersion were proposed
for joint modeling of the mean and dispersion in the analysis of counts
with different levels of dispersion. This class of models allows
modeling the dispersion depending on covariates. The parameters are
estimated by the maximum likelihood method and inferences are made based
on the asymptotic distributions of the estimators. The methodology is
applied to analyze the two count datasets provided by planned
experiments and we compared with double generalized linear models and
the GAMLSS approach. The proposal presented improvements in terms of fit
data when compared to the conventional COM-Poisson with constant
dispersion and was competitive with the double generalized linear models
and GALMSS.

For future work, simulation studies are necessary in order to evaluate
the properties of the estimators and the robustness of the model. In
addition, as an improvement in the case study analysis, considering
nonlinear predictors for the mean can be useful in order to avoid the
use of high-order polynomials and to impose biological restrictions.
