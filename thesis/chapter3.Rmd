...
discussed diagnostics for this class and used a gamma distributed data
as an application. @Andersen2017 extended the double generalized linear
model by considering compound Poisson distributions. Related to discrete
data, @Vieira2011 proposed a Bayesian analysis of the double generalized
linear models for binomial data.
...
briefly describe and commente the double generalized linear models. The
...
analyzed data sets, in the appendix.
...
varying across observations, $\text{Var}(Y_i)=\phi_i V(\mu_i)$. The
model for dispersion is specified by
$$
...
components from the mean submodel. Based on asymptotic distribution
...
in this case, $\bm{V}_{\beta|\gamma} = \bm{V}_{\beta}$ and
$\bm{V}_{\gamma|\beta} = \bm{V}_{\gamma}$, in other words, inference
...
of high an low counts and under- and overdispersion.
...
increases. Thus, this shows the impartiality, consistency, and symmetry
...
the mean regression parameters. However, the likelihoods values are very
...
estimation by using the strategy (a) takes around 30\% more time than
using the strategy (b).
...
$\gamma_1=0$ (moisture effect) andx $\alpha_j = \alpha$ (constant). The
...
component. The results indicate neither under- nor overdispersion for
...
for joint modeling mean and dispersion in the analysis of counts with
...
estimators of the mean and dispersion regression parameters are unbiased
...
The methodology is applied to analyze two count datasets provided by
