...
$\text{Var}(Y) = \phi V(\mu)$. The main examples are $V(\mu) =
\mu(1-\mu)$ for the binomial distribution, $V(\mu) = \mu$ for the
Poisson distribution, $V(\mu) = \mu^2$ for the gamma distribution and
$V(\mu) = \mu^3$ for the inverse-Gaussian distribution
[@McCullagh1989]. This allows modeling specific heterogeneity by
assuming an appropriate distribution. However, once the mean-variance
...
probabilities and facilitating generalizations such as random effects
and modeling of censored data.
